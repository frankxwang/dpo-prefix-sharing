...
class MistralFlexAttention(MistralAttention):
    ...
    def forward(
        ...
        **kwargs,
        ...
        assert (
            q_len % 128 == 0
        ), f"flex_attention requires seq len to be divisble by 128, but got seq of length {q_len}"
        assert (
            attention_mask.shape == (bsz, 1, q_len, q_len) or attention_mask.shape == (1, 1, q_len, q_len)
        ), f"got attention_mask of shape {attention_mask.shape}, expected {(bsz, 1, q_len, q_len)} or {(1, 1, q_len, q_len)}"
